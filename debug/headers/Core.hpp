...
Uint32 flags = SDL_WINDOW_OPENGL | /*SDL_WINDOW_FULLSCREEN |*/ SDL_WINDOW_RESIZABLE |  SDL_WINDOW_SHOWN;

bool initialize(int w = 1280, int h = 768){
	screenWidth  = w;
	screenHeight = h;
	bool success = true;
	if(SDL_Init(SDL_INIT_VIDEO | SDL_INIT_AUDIO) < 0){
		...
		//Cria uma janela OpenGL
		 ...
		 screenWidth  = DM.w;
		 screenHeight = DM.h;*/
		window = SDL_CreateWindow("GL Labs", SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED, screenWidth, screenHeight, flags);
...
extern std::string readShaderSource(const char* pathfile){
...
extern bool createShader(GLuint& program, GLenum shaderType, const std::string& shaderSource){
...
extern bool createProgram(GLuint& program){
...
extern bool linkProgram(GLuint& program){
...
extern void mapBuffer(unsigned int index, size_t buffer_size){
	unsigned int count = 0;
	unsigned int endl  = 0;
	float* ptr = reinterpret_cast<float*>(glMapBuffer(GL_ARRAY_BUFFER + index, GL_READ_ONLY));
	if(ptr != NULL){
		...
		std::cout << "srw::WARNING::The binded buffer is not have data." << std::endl;
	glUnmapBuffer(GL_ARRAY_BUFFER + index);
}

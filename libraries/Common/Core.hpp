...
int ext_screen_width;
int ext_screen_height;
SDL_Window* ext_window;
...
bool initialize(std::string title, int w, int h, bool border = true, bool fullscreen = false){
	...
	if(SDL_Init(SDL_INIT_VIDEO | SDL_INIT_AUDIO) < 0){
		...
		//Cria uma janela OpenGL
		 ...
		 screenWidth  = DM.w;
		 screenHeight = DM.h;*/
		ext_window = SDL_CreateWindow(title.c_str(), SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED, ext_screen_width, ext_screen_height, flags);
		...
		if(ext_window == NULL){
			...
			SDL_SetWindowBordered(ext_window, SDL_bool(border));
			SDL_Cursor* cursor = SDL_CreateSystemCursor(SDL_SYSTEM_CURSOR_ARROW);
			SDL_SetCursor(cursor);
			SDL_ShowCursor(SDL_TRUE);
			//SDL_SetRelativeMouseMode(SDL_TRUE);
			//SDL_SetWindowGrab(window, SDL_TRUE);
			SDL_WarpMouseInWindow(ext_window, ext_screen_width / 2, ext_screen_height / 2);
			//Define o contexto OpenGL
			context = SDL_GL_CreateContext(ext_window);
			if(context == NULL){
